...
public class CalculateSales {
	...
	public static void main(String[] args){
		...
		for(int i = 0; i < (fileNames.size()-1); i++){
			int fileNameA = Integer.parseInt(fileNames.get(i).replaceAll("[^0-9]", ""));
			int fileNameB = Integer.parseInt(fileNames.get(i + 1).replaceAll("[^0-9]", ""));
		...
		try{
			for(int i = 0; i < (fileNames.size()); i++){
				File sales = new File(args[0]+ File.separator + fileNames.get(i));
